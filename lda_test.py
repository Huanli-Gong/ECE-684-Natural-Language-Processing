...
def lda_gen(vocabulary: List[str], alpha: np.ndarray, beta: np.ndarray, xi: int) -> List[str]:
    """Generate a document based on LDA parameters."""
    words = []
    N_d = np.random.poisson(xi)
    theta = np.random.dirichlet(alpha)
    for _ in range(N_d):
        z_n = np.random.choice(len(theta), p=theta)
        w_n = np.random.choice(len(vocabulary), p=beta[z_n])
        words.append(vocabulary[w_n])
    return words
